...
Shows content from [API Ninjas](https://api-ninjas.com) and cycles it with the
**BOOT** button (the non-power button):

1. Joke of the day
2. Trivia of the day (category and question)
3. The trivia answer, then back to 1

Every page has an `Updated <date> <time>` footer showing when the API was last
called. Content is fetched on first boot and once a day after local midnight,
...
src/
  ...
  ninjas_api.c          API Ninjas requests, JSON parsing, text cleanup
  ui.c                  Page layout: header, wrapped body, dots, footer
  content.h             Page enum and the cached content struct
...
cp src/secrets.example.h src/secrets.h   # then set WIFI_SSID, WIFI_PASSWORD
                                         # and NINJAS_API_KEY (api-ninjas.com/profile)
pio run -t upload
pio device monitor
```

`src/secrets.h` is gitignored; the key is compiled into the firmware, so don't
share the built `.bin`.
